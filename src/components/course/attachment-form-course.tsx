...
interface AttachmentFormCourseProps {
...
export const AttachmentFormCourse = ({
  initialData,
  courseId,
}: AttachmentFormCourseProps) => {
  const [isEditing, setIsEditing] = useState(false);
  const [deletingId, setDeltingId] = useState<null | string>(null);
      ...
      {!isEditing && (
        <>
          {initialData.attachments.length === 0 && (
            <p className=" text-sm mt-2 text-zinc-500 italic">
              No attachments yet
            </p>
          )}
          {initialData.attachments.length > 0 && (
